...
const TEMPLATES = [
  {
    ...
    label: "全课业",
    pattern: ["homework"],
    description: "所有空余时段补充课业",
  },
  {
    id: "full-part-time",
    label: "全打工",
    pattern: ["part_time"],
    description: "空余时段安排稳定打工赚灵石",
  },
  {
    id: "full-training",
    label: "全修炼",
    pattern: ["training"],
    description: "把自由时段投入修炼打磨灵力",
  },
  {
    id: "balanced",
    label: "均衡混排",
    pattern: ["homework", "training", "part_time", "walk_city"],
    description: "课业 / 修炼 / 打工 / 社交均衡组合",
  },
];
...
function analyzeMemory(state) {
  ...
  const structureTotals = structures.reduce((acc, node) => {
    if (!node.structure) {
      return acc;
    }
    acc.total += 1;
...
async function runTemplate(page, template, serverUrl) {
  ...
  while (true) {
    ...
    if ([1, 3, 5, 7].includes(state.day)) {
      snapshots[`day${state.day}`] = snapshotFields(state);
    }
    const placed = await placeMemoryPiece(page);
    if (!placed) {
      throw new Error(`Could not find a valid memory placement for day ${state.day}`);
    }
  ...
  try {
    ...
    try {
      for (const template of TEMPLATES) {
        ...
        try {
          const templateResult = await runTemplate(page, template, staticServer.url);
          const memoryLens = analyzeMemory(templateResult.finalState);
          const finalState = templateResult.finalState;
          const finalSnapshot = snapshotFields(finalState);
